...
### Base Counting — Mandatory Pre-Entry Check
Every BUY must include a base count check (see full framework in `memory/base_counting_framework.md`).

| Base | Conviction | Max Position | Rule |
|------|-----------|-------------|------|
| Base 1 | Maximum | 10% | Best R/R — early institutional discovery |
| Base 2 | High | 10% | Sweet spot for super leaders |
| Base 3 | Selective | 7% | Tighten stops to -6%, reduce size |
| Base 4+ | AVOID | 0% | BLOCKED — distribution risk |

**Source:** `scripts/learning/base_counter.py`  
**Cache:** `data/base_counts/{TICKER}.json`  
**Gate position in entry chain:** After Health Check gate, before PULSE gate
...
## Base Counting Framework — Leadership Base Structure Engine

**The base number is the single biggest predictor of breakout success.**

Base counting is embedded into all entry decisions as a mandatory gate in the 6-gate entry chain.

**Source:** `scripts/learning/base_counter.py`  
**Memory file:** `memory/base_counting_framework.md`  
**Cache:** `data/base_counts/{TICKER}.json`

### Base Type Rankings

| Rank | Pattern | Depth | Duration | Win Rate | Notes |
|------|---------|-------|---------|---------|-------|
| 1st | **VCP** (Volatility Contraction) | 10-35% | 3-10 wks | ~70% | Minervini signature — best of all patterns |
| 2nd | **Flat Base** | < 15% | 3+ wks | ~65% | Low volatility = institutional support |
| 3rd | **Cup & Handle** | 15-35% | 7-65 wks | ~60% | Classic — rounded bottom + handle |
| 4th | **IPO Base** | Varies | 3-6 wks | ~55% | First base after IPO lock-up |
| 5th | **High Tight Flag** | < 25% | < 5 wks | ~65% | After 100%+ run — rare, max size |

### Base Quality Score (0–100)

| Factor | Weight | Ideal Signal |
|--------|--------|-------------|
| Volatility Contraction | 25% | Classic VCP (3+ contracting swings) |
| Volume Dry-Up | 20% | Late volume < 50% of early volume |
| Relative Strength | 20% | Stock outperforming SPY during base |
| Tight Closes | 15% | 70%+ bars with < 1.5% daily change |
| Depth | 10% | < 10% depth = perfect flat base |
| Duration | 10% | 3-10 weeks = sweet spot |

### 6-Gate Entry Chain (with Base Count)

```
Phase 3 + score >= 50
  [1] TD Sequential Gate     → no Sell Setup 7-9
  [2] Health Check Gate      → score >= 5/8
  [3] BASE COUNT GATE (NEW)  → Base 4+ = BLOCKED, Base 3 = 70% size
  [4] PULSE Gate             → GREEN or YELLOW
  [5] Cap/Score Gate         → tier-specific score threshold
  [6] BUY
```

### Pivot Rules

- **Pivot** = highest point within base (= breakout trigger)
- **Buy zone** = pivot to pivot × 1.05 (within 5% below, 3% above)
- **Stop loss** = -8% from pivot (Base 3: tighten to -6%)
- **Do NOT chase** more than 3% above pivot — wait for next base or 10EMA pullback
...
## PULSE Architecture — v5.0 (Trend Template as Core)

**PULSE = Trend Template criteria as the PRIMARY axis + supporting quant/fundamental layers.**
The Trend Template is the proven screening backbone (battle-tested, real money). Everything else supports and refines it — NOT replaces it.

**Source (automated daily):** `scripts/pre_compute/trend_template_screener.py`
**Output:** `data/trend_template/screener_latest.json` | Grades A–F shown every morning in session_start

---

### CORE — Trend Template (Minervini SEPA)
*Must evaluate these first, every time, for every stock. These are non-negotiable.*

**Technical Gate** (≥ 4/6 must pass):
| Criterion | Threshold | Superstrong |
|-----------|-----------|-------------|
| % to 52W high | > -20% | — |
| % from 52W low | > +10% | > +15% |
| % from 50D MA | > -5% | — |
| % from 1.5Y high | > 0% | — |
| 150D MA vs 200D MA | > -5% | > 0% (150D above 200D) |
| 6M ADTV | > $15M USD | — |

**RS — Market Gate** (3M + 6M both must pass):
| Criterion | Threshold | Notes |
|-----------|-----------|-------|
| 3M RS percentile vs S&P+Nasdaq | > 70th | > 72nd = STRICT tier |
| 6M RS percentile vs S&P+Nasdaq | > 70th | — |
| RS momentum (3M-6M change) | > -10pp | Negative trend = warning |
| RS momentum (1M-3M change) | PRIMARY | Positive = accelerating leader |

**RS — Sector/Theme Gate** (within-theme percentile):
| Criterion | Threshold | Notes |
|-----------|-----------|-------|
| 3M RS within theme | > 70th | Leader in own sector |
| 6M RS within theme | > 70th | — |
| Theme RS momentum (3M-6M) | > -10pp | Theme losing heat = warning |

**Fundamental Gate** (EPS + Sales both must pass — 5Q trend view):
| Criterion | Threshold | Notes |
|-----------|-----------|-------|
| YoY EPS growth (latest Q) | > +25% | Track 5Q trend |
| YoY Sales growth (latest Q) | > +25% | Track 5Q trend |
| Gross margin change | > 0% | Expansion = pricing power |
| Turnaround label | TURNAROUND / ACCELERATING / STABLE_GROWTH | Setup classification |

**Grading:**
| Grade | Gates Passed | Action |
|-------|-------------|--------|
| A | All 4 gates | Highest conviction — enter when regime allows |
| B | 3 gates (Tech + RS Mkt mandatory) | High priority watchlist |
| C | Tech + RS Mkt only | Monitor — fundamental/theme RS lagging |
| D | Partial | Developing — not ready |
| F | None | Ignore |

**Setup Labels (auto-assigned):**
| Label | Pattern | NRGC Phase |
|-------|---------|------------|
| S1-Leader | ACCELERATING + RS >80th + Phase 2-3 | 3 |
| S2-Turnaround | TURNAROUND label + full pass | 2 |
| S2-Early | EARLY fundamental + tech pass | 1-2 |
| S3-Growth | Revenue accel, Base 0-1 | 2 |
| S1-Watchlist | Full pass, monitor for entry | 2-3 |

---

### SUPPORTING LAYERS
*Used to REFINE entry timing and sizing after Trend Template pass — not as primary screen.*

**Quant/Technical Support:**
1. **Wyckoff × Weinstein** — Stage confirmation (Stage 2 required, Stage 3-4 = NO BUY)
2. **Volume Profile** — POC / HVN = institutional cost basis, accumulation confirmation
3. **SMC** — Entry precision: Liquidity grab → Order Block → CHoCH confirmation
4. **TD Sequential** — Timing gate: Sell Setup 7-9 = BLOCK, Buy Setup 9 = priority entry
5. **Base Count** — Base 1-2 = max size, Base 3 = 70% size, Base 4+ = BLOCKED
6. **VCP Pattern** — Volatility contraction quality score (3+ contracting swings ideal)

**Composite Supporting Scores:**
- **Health Check (8/8)** — Leadership state score: ≥5/8 to enter, ≥7/8 for full size
- **EMLS Score (0-100)** — Earnings-Momentum Leadership composite
- **CANSLIM Score** — C+A+N+S+L+I weighted composite
- **Exhaustion Monitor** — 10-layer exhaustion check (Gate 3 in entry chain)

**Market Context:**
- **M0 Regime** — 7-state market regime: POWER/CONFIRMED/PRESSURE/CHOPPY/CORRECTION/BEAR
- **Theme Heatmap** — 14 themes ranked HOT/WARM/WEAK — hot theme = THEME_BONUS on RS gate
- **NRGC Phase** — Narrative Reflexive Growth Cycle phase (0-7): target Phase 2-3 entry

---

### PULSE Decision Flow (Revised v5.0)

```
STEP 1: TREND TEMPLATE SCREEN (primary filter)
  → Grade A or B = proceed to Step 2
  → Grade C/D/F = watchlist only, no entry

STEP 2: SUPPORTING LAYER CHECK
  → Wyckoff Stage 2 confirmed?         [PASS/FAIL]
  → TD Sequential no Sell Setup 7-9?   [PASS/FAIL]
  → Health Check ≥ 5/8?                [PASS/FAIL]
  → Base Count not 4+?                 [PASS/FAIL]
  → Exhaustion score < 65?             [PASS/FAIL]
  → M0 Regime = buys allowed?          [PASS/FAIL]

STEP 3: POSITION SIZING
  size = base_pct × PULSE_tier_mod × health_mod × base_count_mod × regime_mod
  (Trend Template Grade A = 100% | Grade B = 80% | C = watchlist only)

STEP 4: BUY
  Entry at or within 5% above pivot
  Stop = -8% from entry (Base 3: tighten to -6%)
```
...
**Trend Template pass is necessary but not sufficient — must also pass Wyckoff gate.**
...
## PULSE Screening Thresholds (Trend Template — Core Criteria)

*These are the official PULSE thresholds. Supersedes all prior threshold tables.*
*Source: `scripts/pre_compute/trend_template_screener.py` | Updated 2026-05-16 v5.0*

**Technical (Trend Template Core):**
| Parameter | Min Threshold | Superstrong |
|-----------|--------------|-------------|
| % to 52W high | > -20% | — |
| % from 52W low | > +10% | > +15% |
| % from 50D MA | > -5% | — |
| % from 1.5Y high | > 0% | — |
| 150D MA vs 200D MA | > -5% | > 0% (golden cross) |
| 6M ADTV | > $15M USD | — |

**RS — Market (vs S&P+Nasdaq benchmark, all timeframes):**
| Parameter | Threshold | Notes |
|-----------|-----------|-------|
| 1M RS percentile | > 70th | PRIMARY signal (most recent) |
| 3M RS percentile | > 70th | Must pass for RS Mkt gate |
| 6M RS percentile | > 70th | Must pass for RS Mkt gate |
| RS momentum 1M-3M | > -10pp | Positive = accelerating (KEY) |
| RS momentum 3M-6M | > -10pp | Confirm trend |
| RS momentum hard fail | < -25pp | BLOCKS entry regardless of level |

**RS — Sector/Theme (within-theme percentile):**
| Parameter | Threshold | Notes |
|-----------|-----------|-------|
| 3M within-theme RS | > 70th | Leading in own theme |
| 6M within-theme RS | > 70th | — |
| Theme RS momentum 3M-6M | > -10pp | — |
| Theme RS tier bonus | HOT theme (≥75th) | Lowers RS market threshold to 57th |

**Fundamental (5-Quarter trend — EPS + Sales YoY):**
| Parameter | Threshold | Notes |
|-----------|-----------|-------|
| YoY EPS growth (latest Q) | > +25% | Track 5Q direction |
| YoY Sales growth (latest Q) | > +25% | Track 5Q direction |
| Gross margin change QoQ | > 0% | Expansion preferred |
| Turnaround detection | TURNAROUND / ACCELERATING | Auto-labeled from 5Q pattern |
...
## Data Source Architecture (Real Fund Standard)

yfinance is NOT reliable for a real fund. Use this multi-source failover hierarchy:

| Priority | Source | Use Case | SSL Issues | Cost |
|----------|--------|---------|-----------|------|
| 1st | **Polygon.io** | Price OHLCV, quotes | None | Free EOD / $29-79/mo |
| 2nd | **Tiingo** | Price backup | None | Free / $10/mo |
| 3rd | **FMP (Financial Modeling Prep)** | Fundamentals, earnings, revenue | None | Free 250/day / $15/mo |
| 4th | **Finnhub** | Market cap (current) | None | Free 60/min |
| 5th | **query2.finance.yahoo.com** | Emergency fallback | verify=False | Free |
| Macro | **FRED** | Yields, DXY, macro | None | Free |
| Thai | **SET MCP** | Thai stocks | None | Configured |

**Implementation:** `scripts/agents/data_engine.py` (to be built)  
**Full spec:** `memory/agent_architecture_trading.md`

**Required .env keys to add:**
```
POLYGON_API_KEY=    # polygon.io — free signup, no CC
TIINGO_API_KEY=     # api.tiingo.com — free signup
FMP_API_KEY=        # financialmodelingprep.com — free 250/day
```

---

## Sub-Agent Architecture: Buy/Sell Execution System

10 specialized sub-agents for real fund operation. Full spec in `memory/agent_architecture_trading.md`.

### Core Trading Agents
| Agent | File | Purpose |
|-------|------|---------|
| **D1 Data Engine** | `agents/data_engine_agent.py` | Multi-source data with validation + failover |
| **D2 Signal Engine** | `agents/signal_engine_agent.py` | All indicators: TD, exhaustion, RS, HC, base |
| **T1 BUY Agent** | `agents/buy_agent.py` | 8-gate entry chain, structured buy_order output |
| **T2 SELL Agent** | `agents/sell_agent.py` | 9-check priority exit system, sell_order output |
| **T3 Risk Guardian** | `agents/risk_guardian_agent.py` | Portfolio-level gate — reviews every trade |
| **W1 Watchlist** | `agents/watchlist_agent.py` | NRGC phase monitoring, Phase 2→3 alerts |
| **M2 Exhaustion Monitor** | `agents/exhaustion_monitor_agent.py` | 10-layer exhaustion score, daily |
| **X1 Paper Execution** | (existing auto_trader.py) | Writes to portfolio_state.json |
| **X2 Real Execution** | `agents/real_execution_agent.py` | Alpaca/IBKR broker API (future) |

### 8-Gate Entry Chain (BUY Agent)
```
[1] Market Regime Gate  — Confirmed Uptrend only (no buys in correction)
[2] TD Sequential Gate  — No Sell Setup 7-9 or Countdown 10-13
[3] Exhaustion Gate     — 10-layer score must be < 65
[4] Health Check Gate   — Score >= 5/8
[5] Base Count Gate     — Base 4+ = BLOCKED
[6] PULSE Gate          — RED = BLOCKED
[7] Cap/Score Tier      — Mega/Large require higher score threshold
[8] Liquidity Gate      — ADTV >= $5M
```

### Priority-Ordered SELL Triggers (SELL Agent)
```
IMMEDIATE: Hard stop -8% | Phase 6/7 | Climax RSI>85 | Guidance cut | Gap-down -10%
TODAY:     TD Sell Setup 9 | Exhaustion > 65 | HC drops to Red | TF drops to 2/4
REVIEW:    TD Countdown 10+ | RS deteriorating | Breaks 21EMA | Base failure
PROFIT:    Trail at +30% | Partial at +20% (fast move) | TD13 on stock = exit 75%
```
